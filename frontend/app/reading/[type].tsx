...
export default function ReadingScreen() {
  ...
  const [consent, setConsent] = useState<{ personalized: boolean } | null>(null);
  useEffect(() => { (async () => setConsent(await initConsent()))(); }, []);
  ...
  const renderCard = (cardData: ReadingCard, index: number) => {
    const animatedStyle = cardAnimations[index] ? {
      ...
      transform: [
        { translateY: cardAnimations[index].interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) },
    ...
    return (
      <Animated.View key={index} style={[styles.cardContainer, animatedStyle]}>
        <View style={[styles.card, cardData.reversed && styles.cardReversed]}>
          <LinearGradient colors={['#2D1B69', '#1A1A2E']} style={styles.cardGradient}>
          ...
          {!showCards ? (
            <View style={styles.setupContainer}>
              <View style={styles.languageContainer}>
                <TouchableOpacity style={styles.languageButton} onPress={toggleLanguage} activeOpacity={0.7}>
                  <Ionicons name="language" size={20} color="white" />
                  <Text style={styles.languageText}>{language === 'en' ? 'TR' : 'EN'}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.backButtonContainer}>
                <TouchableOpacity style={styles.backButton} onPress={() => router.push('/')} activeOpacity={0.7}>
                  ...
                  <Text style={styles.backButtonText}>{language === 'en' ? 'Home' : 'Ana Sayfa'}</Text>
                </TouchableOpacity>
              </View>

              <LinearGradient colors={['#4C63D2', '#7C4DFF']} style={styles.headerGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              ...
              <TouchableOpacity style={styles.startButton} onPress={handleStartReading} disabled={loading}>
                <LinearGradient colors={['#4C63D2', '#7C4DFF']} style={styles.startButtonGradient}>
                  {loading ? (
            ...
            <View style={styles.readingContainer}>
              ...
              <Text style={styles.readingResultTitle}>{t.yourReading.replace('{readingName}', readingConfig.name)}</Text>

              <View style={styles.cardsDisplay}>{reading.cards.map((cardData, index) => renderCard(cardData, index))}</View>
              ...
              {gated && (
                <Pressable onPress={() => setShowPaywall(true)} testID="cta-premium-detail" style={{ marginTop: 12, padding: 12, borderRadius: 12, borderWidth: 1 }}>
                  <Text>Daha detaylı yorum için Premium</Text>
                </Pressable>
              )}
              ...
              <TouchableOpacity style={styles.newReadingButton} onPress={() => { setShowCards(false); setReading(null); setQuestion(''); }}>
                <LinearGradient colors={['#4A4A4A', '#2A2A2A']} style={styles.newReadingButtonGradient}>
...
const styles = StyleSheet.create({
  ...
  background: { flex: 1 },
  languageContainer: { alignItems: 'flex-end', paddingHorizontal: 20, paddingTop: 10 },
  languageButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
  languageText: { color: 'white', fontSize: 14, fontWeight: '600', marginLeft: 6 },
  backButtonContainer: { paddingHorizontal: 20, paddingTop: 10, paddingBottom: 10 },
  backButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 25, alignSelf: 'flex-start' },
  backButtonText: { color: 'white', fontSize: 14, fontWeight: '600', marginLeft: 8 },
  setupContainer: { flex: 1, padding: 20 },
  headerGradient: { padding: 24, borderRadius: 16, marginBottom: 30, alignItems: 'center' },
  readingTitle: { fontSize: 28, fontWeight: 'bold', color: 'white', textAlign: 'center', marginBottom: 12 },
  readingDescription: { fontSize: 16, color: 'rgba(255,255,255,0.9)', textAlign: 'center', lineHeight: 24 },
  questionContainer: { marginBottom: 30 },
  questionLabel: { fontSize: 18, color: 'white', marginBottom: 12, fontWeight: '600' },
  questionInput: { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: 16, color: 'white', fontSize: 16, minHeight: 100, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  startButton: { borderRadius: 16, overflow: 'hidden', elevation: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
  startButtonGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 18 },
  startButtonText: { color: 'white', fontSize: 18, fontWeight: '600', marginLeft: 8 },
  readingContainer: { padding: 20 },
  readingResultTitle: { fontSize: 24, fontWeight: 'bold', color: 'white', textAlign: 'center', marginBottom: 30 },
  cardsDisplay: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', marginBottom: 30 },
  cardContainer: { alignItems: 'center', margin: 8 },
  card: { width: CARD_WIDTH, height: CARD_HEIGHT, borderRadius: 12, overflow: 'hidden', elevation: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
  cardReversed: { transform: [{ rotateX: '180deg' }] },
  cardGradient: { flex: 1, padding: 12, justifyContent: 'space-between', alignItems: 'center' },
  cardNumber: { fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: '600' },
  cardName: { fontSize: 12, color: 'white', textAlign: 'center', fontWeight: '500', lineHeight: 16 },
  reversedLabel: { fontSize: 10, color: '#FF6B6B', fontWeight: '600' },
  positionLabel: { fontSize: 12, color: 'rgba(255,255,255,0.8)', textAlign: 'center', marginTop: 8, fontWeight: '500' },
  interpretationContainer: { backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 20, marginBottom: 30 },
  interpretationTitle: { fontSize: 20, color: 'white', fontWeight: '600', marginBottom: 16, textAlign: 'center' },
  interpretationText: { fontSize: 14, color: 'rgba(255,255,255,0.9)', lineHeight: 22 },
  newReadingButton: { borderRadius: 12, overflow: 'hidden' },
  newReadingButtonGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16 },
  newReadingButtonText: { color: 'white', fontSize: 16, fontWeight: '600', marginLeft: 8 },
